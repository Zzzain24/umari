  ...
  return (
    <section className="relative py-24 px-4">
      <div className="max-w-7xl mx-auto">
          ...
          >
            <button
              ...
              className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-200 ${
                !isAnnual ? "bg-[#e78a53] text-white shadow-lg" : "text-black hover:text-black/80"
              }`}
            >
              Monthly
            </button>
          ...
          {pricingPlans.map((plan, index) => (
            <motion.div
              ...
              className={`relative rounded-2xl p-8 backdrop-blur-sm border-2 transition-all duration-300 ${
                plan.popular
                  ? "bg-gradient-to-b from-[#e78a53]/10 to-transparent border-[#e78a53]/30 shadow-lg shadow-[#e78a53]/10 hover:border-[#e78a53]/60 hover:shadow-[#e78a53]/20"
                ...
                <div className="flex items-baseline justify-center gap-1 mb-2">
                  {plan.price ? (
                    ...
                    <>
                      ...
                      <span className="text-black/60 text-lg">/transaction</span>
                    </>
                  ) : (
                    <>
                      <span className="text-4xl font-bold text-black">
                      ...
                      <span className="text-black/60 text-lg">{isAnnual ? "/year" : "/month"}</span>
                    </>
                  )}
                </div>
                <p className="text-black/60 text-sm">{plan.description}</p>
              </div>
        ...
        >
          <p className="text-black/60 mb-4">Need a custom solution? We're here to help.</p>
          <motion.a
